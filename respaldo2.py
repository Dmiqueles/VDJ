que esta mal? porque no funciona bien? podria el codigo calcular el tiempo exacto que falta en cada bloque para dejar de tener problemas?
        ...
        for row in data:
            try:
                ...
                promos.append({
                    ...
                    'type': 'Promo'  # Asegurar este campo
                })
            except ValueError:
        ...
        for row in data:
            try:
                ...
                fillers.append({
                    ...
                    'duration': duration_seconds,
                    'duration_seconds': duration_seconds,
                    'type': 'Filler'  # Asegurar este campo
                })
            except ValueError:
...
# Inicializar last_used en el estado de la sesión
...
def add_promos_and_fillers(block_special, time_remaining, promos, fillers, item_counter, current_time):
    # Encontrar la mejor combinación de promos y rellenos sin preocuparse por repeticiones recientes
...
def generate_playlist(start_time, end_time, promos, fillers, user_programs, selected_sheet, include_block_zero=True):
    ...
    while user_programs:
        ...
        if remaining_block_time > 0:
            # Intentar llenar el tiempo restante con promos y rellenos
            combination = find_optimal_combination(remaining_block_time, promos + fillers)
            if combination:
                for content in combination:
                    content_type = 'Promo' if content in promos else 'Filler'
                ...
                # Si no hay combinación posible, crear una tanda parcial (último recurso)
                if remaining_block_time > 0:
                    tanda_duration = min(remaining_block_time, 30)  # Máximo 30s
                    block.append({
                        "item": item_counter,
                        "start_time": current_time.strftime("%H:%M:%S"),
                        "name": f"TANDA PARCIAL {int(tanda_duration)}s",
                        "duration": format_duration(tanda_duration),
                        "duration_seconds": tanda_duration,
                        "type": "Tanda Parcial",
                        "block": len(initial_blocks) + 1,
                        "relleno": selected_sheet,  # Agregar el relleno seleccionado
                    })
                    ...
                    remaining_block_time -= tanda_duration

                    # Intentar llenar tiempo restante nuevamente
                    if remaining_block_time > 0:
                        combination = find_optimal_combination(remaining_block_time, promos + fillers)
                        if combination:
                            for content in combination:
    ...
    while current_time < end_datetime:
        ...
        # Calcular la hora de inicio del siguiente bloque
        next_block_start_time = calculate_time_to_next_block(current_time)
        if current_time + timedelta(seconds=block_duration) > end_datetime:
            break

        # Ajustar la hora de inicio del bloque
        if current_time < next_block_start_time:
    ...
    if time_remaining > 0:
        ...
        if combination:
            ...
            # Si no hay combinación posible, crear una tanda parcial (último recurso)
            if time_remaining > 0:
                tanda_duration = min(time_remaining, 30)  # Máximo 30s
                block_special.append({
                    "item": item_counter,
                    "start_time": current_time.strftime("%H:%M:%S"),
                    "name": f"TANDA PARCIAL {int(tanda_duration)}s",
...
def find_optimal_combination(time_available, content_pool):
    # Eliminar cualquier rastro de last_used
    best_combination = []
    best_score = -1
    
    # Mezclar el contenido para evitar patrones repetitivos
    shuffled = sorted(content_pool, key=lambda x: x['duration_seconds'], reverse=True)
    
    # Probar diferentes estrategias de combinación
    for strategy in [True, False]:  # Largos primero vs cortos primero
        current_combo = []
        remaining = time_available
        
        for item in sorted(shuffled, key=lambda x: x['duration_seconds'], reverse=strategy):
            if item['duration_seconds'] <= remaining:
        ...
        # Calcular puntaje (priorizar máximo tiempo usado)
        time_used = sum(c['duration_seconds'] for c in current_combo)
        score = time_used * 1000  # Priorizar tiempo sobre balance
        
        if score > best_score:
            best_combination = current_combo
            best_score = score

    return best_combination if best_score > 0 else None
