...
# Función corregida para cargar programas
    ...
    try:
        ...
        for row in data:
            try:
                ...
                programs.append({
                    'name': row['Name'], 
                    'duration_seconds': duration_seconds  # Clave corregida
                })
                
            except ValueError as e:
                error_msg = f"Duración inválida en programa {row.get('Name', 'Desconocido')}: {row.get('Duration', 'Sin duración')}"
                st.session_state.messages.append({"type": "error", "content": error_msg})
        
        if programs:
            st.session_state.messages.append({"type": "success", 
                "content": f"Programas cargados: {len(programs)} desde {sheet_name}"})
            
        return programs
        
    except Exception as e:
        error_msg = f"Error al cargar programas desde {sheet_name}: {str(e)}"
        st.session_state.messages.append({"type": "error", "content": error_msg})
        return []
    ...
    try:
        ...
        for row in data:
            try:
                ...
                promos.append({
                    'name': row['Name'], 
                    'duration_seconds': duration_seconds,  # Cambiado de 'duration' a 'duration_seconds'
                    'type': 'Promo'
                })
    ...
    try:
        ...
        for row in data:
            try:
                ...
                error_msg = f"Duración inválida en {row.get('Name', 'Desconocido')}: {row.get('Duration', 'Sin duración')}"
                st.session_state.messages.append({"type": "error", "content": error_msg})
        
        if programs:
            success_msg = f"✅ Programas cargados: {len(programs)} desde {sheet_name}"
            st.session_state.messages.append({"type": "success", "content": success_msg})
        
        return programs  # <--- Retorno dentro del try
        
    except Exception as e:  # <--- Único except para errores generales
        error_msg = f"❌ Error al cargar programas: {str(e)}"
        st.session_state.messages.append({"type": "error", "content": error_msg})
        return []
...
def select_program(user_programs, last_program, time_remaining):
    for program in user_programs:
        # Acceder a duration_seconds en lugar de duration
...
def add_promos_and_fillers(block_special, time_remaining, promos, fillers, item_counter, current_time):
    # Primero agregar todas las promos posibles
    for promo in sorted(promos, key=lambda x: -x['duration_seconds']):
        if time_remaining <= 0:
            break
        if promo['duration_seconds'] <= time_remaining:
            block_special.append({
                "item": item_counter,
                "start_time": current_time.strftime("%H:%M:%S"),
                "name": promo['name'],
                "duration": format_duration(promo['duration_seconds']),
                "duration_seconds": promo['duration_seconds'],
                "type": "Promo",
                "block": "Especial"
            })
            item_counter += 1
            current_time += timedelta(seconds=promo['duration_seconds'])
            time_remaining -= promo['duration_seconds']
    
    # Luego llenar con fillers si aún queda tiempo
    for filler in sorted(fillers, key=lambda x: -x['duration_seconds']):
        if time_remaining <= 0:
            break
        if filler['duration_seconds'] <= time_remaining:
            block_special.append({
                "item": item_counter,
                "start_time": current_time.strftime("%H:%M:%S"),
                "name": filler['name'],
                "duration": format_duration(filler['duration_seconds']),
                "duration_seconds": filler['duration_seconds'],
                "type": "Filler",
                "block": "Especial"
            })
            item_counter += 1
            current_time += timedelta(seconds=filler['duration_seconds'])
            time_remaining -= filler['duration_seconds']
    
    return block_special, item_counter, current_time, time_remaining

def generate_playlist(start_time, end_time, promos, fillers, user_programs, include_block_zero=True, is_night_schedule=False):
    def format_duration(s):
        if not isinstance(s, (int, float)) or s < 0:
            return "00:00:00"
        return f"{int(s//3600):02d}:{int((s//60)%60):02d}:{int(s%60):02d}"
    ...
    while user_programs:
        ...
        if remaining_block_time > 0:
            # Limitar la tanda parcial a un máximo de 30 segundos
            tanda_parcial_duration = min(remaining_block_time, 30)
            block.append({
                "item": item_counter,
                "start_time": current_time.strftime("%H:%M:%S"),
                "name": f"Tanda Parcial ({int(tanda_parcial_duration)}s)",
                "duration": format_duration(tanda_parcial_duration),
                "duration_seconds": tanda_parcial_duration,
                "type": "Tanda Parcial",
                "block": len(initial_blocks) + 1
            })
            item_counter += 1
    ...
    # Dentro de generate_playlist, reemplazar el bloque especial con:
    time_remaining = (end_datetime - current_time).total_seconds()
    if time_remaining > 0:
        block_special = []
        
        # Priorizar promos antes que fillers
        for promo in sorted(promos, key=lambda x: -x['duration_seconds']):
            if time_remaining <= 0:
                break
            if promo['duration_seconds'] <= time_remaining:
        ...
        # Si todavía queda tiempo, agregar fillers
        for filler in sorted(fillers, key=lambda x: -x['duration_seconds']):
            if time_remaining <= 0:
                break
            if filler['duration_seconds'] <= time_remaining:
...
def find_optimal_combination(time_available, content_pool):
    sorted_content = sorted(
        [c for c in content_pool if c.get('duration_seconds', 0) <= time_available],
        key=lambda x: (-x['duration_seconds'], x['type'] == 'Filler')
    )
    ...
    for content in sorted_content:
        if content['duration_seconds'] <= remaining_time:
            combination.append(content)
            remaining_time -= content['duration_seconds']
...
def main():
    if 'mode' not in st.session_state:
        st.session_state.mode = "Modo Simple"  # O el valor por defecto que prefieras
    ...
    with st.sidebar:
        ...
        new_mode = st.radio(
            ...
            index=0 if st.session_state.get('mode') == "Modo Simple" else 1
        )
    ...
    # ------------------------------------------------------
    if st.session_state.get('mode') == "Modo Simple":
        ...
        if st.button("✨ Generar Playlist", type="primary", use_container_width=True):
            ...
            with st.spinner("🔍 Cargando programas, promos y rellenos..."):
                ...
                if not day_programs or not promos or not fillers:
                    ...
                    # Generar playlist
                    playlist = generate_playlist(start_time, end_time, promos, fillers, day_programs)
            ...
            if st.button("✨ Generar Playlist", type="primary", use_container_width=True):
                ...
                with st.spinner("🔍 Cargando programas, promos y rellenos..."):
                    ...
                    if not day_programs or not promos or not fillers:
                        ...
                        day_playlist = generate_playlist(day_start_time, day_end_time, promos, fillers, day_programs)
                        
                        # Generar playlist nocturna (si está activado)
                        night_playlist = []
                        if use_night_schedule:
                            # Dentro del bloque donde generas la night_playlist en el Modo Completo:
                            night_playlist = generate_playlist(
                                night_start_time,
                                night_end_time,
                                promos,
                                fillers,
